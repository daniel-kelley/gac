...
class GAC::Panel
  ...
  #
  def generate(desc)
    panel = File.open(desc) { |yf| YAML.safe_load(yf) }
    analyze_panel(panel)
    collect_io_types
    register_block_io
  ...
  def panel_header(desc)
    <<~PANEL
      // GAC Panel Generated by #{$PROGRAM_NAME} from #{desc}
      gac = library("gac.lib");
      ...
      SIGNAL_MIN = CONTROL_MIN;
      SIGNAL_MAX = CONTROL_MAX;
      SIGNAL_STEP = CONTROL_STEP;
      SIGNAL_DEFAULT = CONTROL_DEFAULT;
      signal2freq(n) = 1000*n;
    PANEL
  end
  ...
  def panel_process
    ...
    s << @output_names.join(",\n")
    s << ') = ('
    s << @output_names.select { |n| n =~ /^output/ }.join(",\n")
    s << ');'
